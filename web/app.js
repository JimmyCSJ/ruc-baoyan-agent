...
function collectLongPlanTimelineItems() {
  ...
  (Array.isArray(report.timeline) ? report.timeline : []).forEach((block, idx) => {
    ...
    const rawTasks = block.core_tasks;
    ...
    const taskList = Array.isArray(rawTasks)
      ? rawTasks.map((x) => String(x || "").trim()).filter(Boolean)
      : String(rawTasks || "")
          .split(/\n+|[；;](?=\s*(?:20\d{2}|\d{1,2}月|即日起|本周|今天|立即))/)
          .map((x) => x.replace(/^[-*•\d.、\s]+/, "").trim())
          .filter(Boolean);
    if (taskList.length) {
      taskList.forEach((task, taskIdx) => {
        const text = [bucket, title, task].filter(Boolean).join("：");
        timeline.push({
          text,
          title: task.slice(0, 44) || title || bucket || `阶段 ${idx + 1}`,
          deadline_or_window: task || deadline_or_window,
          date: timelineDateForItem({ ...block, deadline_or_window: task || deadline_or_window, text: task }, idx + taskIdx),
          source: "timeline_task",
        });
      });
      return;
    }
    const text = [bucket, title].filter(Boolean).join("：");
    if (text) {
      timeline.push({
        text,
...
function collectLongPlanCalendarItems() {
  const timelineItems = collectLongPlanTimelineItems();
  if (timelineItems.length) return timelineItems.slice(0, 24);
  return collectLongPlanTodoItems().slice(0, 24);
}
...
async function addLongPlanToCalendar() {
  ...
  try {
    ...
    const newPlans = calendarItems.slice(0, 24).map((item, idx) => ({
      id: `lp_${appNowDate().getTime().toString(36)}_${idx}`,
...
async function addLongPlanToTodos() {
  ...
  try {
    ...
    const now = appNowDate();
    const createdAt = now.toISOString();
    const newItems = todoItems.slice(0, 24).map((item, idx) => ({
      id: `lt_${appNowDate().getTime().toString(36)}_${idx}`,
...
ensureAuthenticated().then((ok) => {
  ...
  setActiveView(views[requestedView] ? requestedView : "calendar");
});
