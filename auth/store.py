...
def _study_calendars_dir() -> Path:
    return _data_root() / "study_calendars"


def _todos_dir() -> Path:
    return _data_root() / "todos"
...
def _ensure_dirs() -> None:
    ...
    _study_calendars_dir().mkdir(parents=True, exist_ok=True)
...
def study_calendar_path(username: str) -> Path:
    safe = re.sub(r"[^\w\-]", "_", username)
    return _study_calendars_dir() / f"{safe}.json"


def load_study_calendar(username: str) -> Dict[str, Any]:
    path = study_calendar_path(username)
    data = _read_json(path, {})
    return data if isinstance(data, dict) else {}


def save_study_calendar(username: str, calendar: Dict[str, Any]) -> Dict[str, Any]:
    with _LOCK:
        path = study_calendar_path(username)
        calendar = dict(calendar)
        calendar["updated_at"] = _now_iso()
        _write_json(path, calendar)
        return calendar


def todos_path(username: str) -> Path:
    safe = re.sub(r"[^\w\-]", "_", username)
    return _todos_dir() / f"{safe}.json"


def load_todos(username: str) -> Dict[str, Any]:
    path = todos_path(username)
    data = _read_json(path, {})
    return data if isinstance(data, dict) else {}


def save_todos(username: str, todos: Dict[str, Any]) -> Dict[str, Any]:
    with _LOCK:
        path = todos_path(username)
        todos = dict(todos)
        todos["updated_at"] = _now_iso()
