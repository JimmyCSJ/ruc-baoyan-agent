...
def _part_system_prompt(part: int, sk: Dict[str, Any], kb_has: bool) -> str:
    ...
    extras = {
        ...
        4: (
            "本段对应「四、核心行动指南」。action_guidelines 为 5～10 条可执行字符串。\n"
        ),
        5: (
...
def generate_long_plan_section(
    ...
    head = (
        ...
        "时间理解要求：规划生成时间就是本次规划的起点。若用户年级为大三，或准备进度已进入夏令营/预推免窗口，"
        "必须按大三下学期的保研节奏安排，5月-6月中旬视为夏令营材料与投递筹备期，"
        "不得写成“还有一年再准备”或按大二长期探索节奏输出。\n"
        "时间边界要求：不得引用、假设或提前使用规划生成时间之后才发布的通知、简章、经验帖或统计资料；"
        "如果当年文件截至规划起点尚未发布，必须写明需要后续核验，不能伪装成已知事实。\n"
    )
...
def _references_html(report: Dict[str, Any]) -> str:
    ...
    def ref_index(ref: Dict[str, Any]) -> str:
        return str(ref.get("idx") or ref.get("index") or "").strip()

    def ref_entry(ref: Dict[str, Any]) -> str:
        return str(ref.get("entry") or ref.get("title") or "").strip()

    def ref_url(ref: Dict[str, Any]) -> str:
        return str(ref.get("url") or "").strip()

    def is_authoritative(ref: Dict[str, Any]) -> bool:
        text = " ".join(
            str(ref.get(k) or "")
            for k in ("entry", "title", "url", "source_group", "source")
        ).lower()
        return any(
            marker in text
            for marker in (
    ...
    authoritative = [r for r in normalized if is_authoritative(r)]
    other = [r for r in normalized if r not in authoritative]
    featured = (authoritative + other)[:6]
    remaining = (authoritative + other)[6:]

    def item_html(ref: Dict[str, Any]) -> str:
        idx = _html_escape(ref_index(ref))
        entry = _html_escape(ref_entry(ref))
        url = ref_url(ref)
        if not entry:
            return ""
        if url:
    ...
    featured_items = [item_html(r) for r in featured]
    featured_items = [x for x in featured_items if x]
    remaining_items = [item_html(r) for r in remaining]
    remaining_items = [x for x in remaining_items if x]
    ...
    if remaining_items:
        ...
        <details class="reference-details">
          <summary>展开查看其余 {len(remaining_items)} 条检索资料</summary>
          <ol class="reference-list reference-list-secondary">{''.join(remaining_items)}</ol>
        </details>
        """
    return f"""
      <section class="references-section">
        <h2 class="section-title">参考链接与资料来源</h2>
        <p class="reference-stat">本次长程规划共参考 {len(normalized)} 条资料，重点展示 {len(featured_items)} 条高相关资料，其余 {len(remaining_items)} 条收起备查。</p>
        <p class="reference-note">优先展示官方通知和与目标项目直接相关的资料；经验帖和批量检索结果收起，避免干扰阅读。</p>
        <ol class="reference-list reference-list-featured">{''.join(featured_items)}</ol>
        {details}
      </section>
    """
...
def _report_meta_date(report: Dict[str, Any]) -> str:
    ...
    if not gen:
        fixed = os.getenv("APP_FIXED_NOW") or os.getenv("LONG_PLAN_FIXED_NOW")
        if fixed:
            try:
                return datetime.fromisoformat(fixed.replace("Z", "+00:00")).strftime("%Y年%m月%d日")
            except ValueError:
                pass
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .reference-stat {{ margin:0 0 8px; padding:12px 14px; border:1px solid var(--line); border-radius:10px; background:#fff7f8; color:#344054; font-weight:800; }}
    .reference-note {{ color:#475467; font-size:14px; margin:0 0 12px; }}
    ...
    .reference-details {{ margin-top:12px; padding:12px 14px; border:1px solid var(--line); border-radius:10px; background:#fafafa; }}
    .reference-details summary {{ cursor:pointer; font-weight:800; color:#344054; }}
    .reference-list-secondary {{ margin-top:10px; max-height:320px; overflow:auto; }}
...
<body>
  <article class="report-shell">
    ...
    <main>
      <div class="summary-strip"><span><strong>目标：</strong>{target}</span><span><strong>用途：</strong>把保研方向、材料准备和阶段任务整理成一份可执行路线图</span></div>
      {numbered_sections}
      {references_section}
      <div class="quality">使用提醒：本规划适合作为接下来一年保研准备的路线图。公开经验可以帮助判断方向，但正式报名条件、考核方式和时间安排仍应以学院最新通知为准。</div>
      <p class="footer-note">资料依据包括官方文件、公开经验资料和个人画像信息；经验信息可能过时或带有个人偏差，请以中国人民大学各学院官网及研究生招生网最新通知为准。</p>
    </main>
  </article>
</body>
</html>"""
