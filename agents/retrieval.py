...
def _retrieval_cutoff() -> datetime | None:
    if not _env_bool("ENABLE_RETRIEVAL_TIME_CUTOFF", True):
        return None
    raw = (os.getenv("APP_FIXED_NOW") or os.getenv("LONG_PLAN_FIXED_NOW") or "").strip()
    if not raw:
        return None
    try:
...
def _parse_date_candidate(value: object) -> datetime | None:
...
def _doc_file_mtime(doc: RetrievedDoc) -> datetime | None:
    prov = doc.get("provenance") if isinstance(doc.get("provenance"), dict) else {}
    file_value = str((prov or {}).get("file") or "").strip()
    if not file_value:
        return None
...
def _drop_reason_after_cutoff(doc: RetrievedDoc, cutoff: datetime | None) -> str:
    if cutoff is None:
        return ""
    prov = doc.get("provenance") if isinstance(doc.get("provenance"), dict) else {}
    ...
    for key in (
        ...
        dt = _parse_date_candidate((prov or {}).get(key))
        if dt:
            candidates.append(dt)
    if str(doc.get("source_group") or "") == "web":
        for value in (doc.get("title"), doc.get("content"), (prov or {}).get("url")):
            dt = _parse_date_candidate(value)
            if dt:
                candidates.append(dt)
    mtime = _doc_file_mtime(doc)
    if mtime:
        candidates.append(mtime)
    if candidates and min(candidates) > cutoff:
        return "资料日期晚于演示当前日期"
    if str(doc.get("source_group") or "") == "web" and not candidates and _env_bool("DROP_UNDATED_WEB_AFTER_FIXED_NOW", True):
        return "联网资料未标明发布日期，演示时间下不使用"
    return ""


def _filter_docs_by_cutoff(docs: List[RetrievedDoc]) -> Tuple[List[RetrievedDoc], List[RetrievedDoc]]:
    cutoff = _retrieval_cutoff()
    if cutoff is None:
        return docs, []
    kept: List[RetrievedDoc] = []
    dropped: List[RetrievedDoc] = []
    for doc in docs:
        reason = _drop_reason_after_cutoff(doc, cutoff)
        if reason:
            d: RetrievedDoc = dict(doc)  # type: ignore[misc]
            prov = dict(d.get("provenance") or {})
            prov["time_cutoff_drop_reason"] = reason
            prov["time_cutoff"] = cutoff.strftime("%Y-%m-%d")
            d["provenance"] = prov
            dropped.append(d)
        else:
            kept.append(doc)
...
def _append_cutoff_step(execution_steps: List[str], dropped: List[RetrievedDoc], label: str) -> None:
    if not dropped:
        return
    cutoff = _retrieval_cutoff()
    cutoff_text = cutoff.strftime("%Y-%m-%d") if cutoff else "演示日期"
    execution_steps.append(f"时间过滤：{label}中剔除 {len(dropped)} 条晚于 {cutoff_text} 或日期不明的资料，避免提前读取后续信息。")
    ...
    *,
    ...
    if ok > 0:
        ...
        if selected:
            ...
            official_docs = search_official(retrieval_query, ok)
        official_docs, dropped = _filter_docs_by_cutoff(official_docs)
        _append_cutoff_step(execution_steps, dropped, "官方材料")
        allowed_files = {str((d.get("provenance") or {}).get("file") or "") for d in official_docs}
        official_files_read = [f for f in official_files_read if f in allowed_files]
    experience_docs = search_experience(retrieval_query, ek) if ek > 0 else []
    experience_docs, dropped = _filter_docs_by_cutoff(experience_docs)
    _append_cutoff_step(execution_steps, dropped, "公众经验库")
    ...
    if kb_scope in ("hybrid", "public_only") and query_plan.get("needs_basics"):
        ...
        basics_docs, dropped = _filter_docs_by_cutoff(basics_docs)
        _append_cutoff_step(execution_steps, dropped, "保研通识库")
        if basics_docs:
    ...
    if web_allowed:
        ...
        web_docs, dropped = _filter_docs_by_cutoff(web_docs)
        _append_cutoff_step(execution_steps, dropped, "联网结果")
    ...
    merged, dropped = _filter_docs_by_cutoff(merged)
    _append_cutoff_step(execution_steps, dropped, "最终资料包")
