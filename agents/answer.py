...
def current_date_key() -> str:
    """Return the current local date key for references."""
    fixed = os.getenv("APP_FIXED_NOW") or os.getenv("LONG_PLAN_FIXED_NOW")
    if fixed:
        try:
            return datetime.fromisoformat(fixed.replace("Z", "+00:00")).strftime("%Y-%m-%d")
        except ValueError:
            pass
...
def generate_llm_answer(
    ...
    prompt = (
        ...
        "若资料年份早于当前日期，需说明可能过时。\n"
        "时间边界（强制）：不得引用、假设或提前使用当前日期之后才发布的通知、简章、经验帖或统计资料；"
        "若问题需要后续文件才能确认，必须写明“截至当前日期尚未确认”。\n"
