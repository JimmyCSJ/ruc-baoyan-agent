...
def _source_available_at_fixed_time(path: Path) -> bool:
    raw = (os.getenv("APP_FIXED_NOW") or os.getenv("LONG_PLAN_FIXED_NOW") or "").strip()
    if not raw or not path.is_file():
        return True
    try:
        cutoff = datetime.fromisoformat(raw.replace("Z", "+00:00")).replace(tzinfo=None)
    except ValueError:
        return True
...
def data_agent_query(user_query: str) -> Tuple[str, bool]:
    ...
    stats_path = Path(__file__).resolve().parent.parent / "data" / "public_info_manual_stats" / "ruc_2026_manual_stats.txt"
    if not _source_available_at_fixed_time(stats_path):
        return "", False
    try:
        build_stats_db()
